...
def prepare_input(img: np.ndarray, size: int = MODEL_INPUT_SIZE) -> torch.Tensor:
    """Resize HWC [0,1] image to model input size and return CHW tensor."""
...
def main() -> None:
    ...
    for scene_name, flash_file, noflash_file, ref_files in REAL_SCENES:
        ...
        flash_t = prepare_input(flash_full).unsqueeze(0).to(device)
        noflash_t = prepare_input(noflash_full).unsqueeze(0).to(device)

        output, gates = model(flash_t, noflash_t)
        output_np = tensor_to_numpy(output.squeeze(0))

        # Load reference results (resized to 512 for comparison)
        extra_refs = {}
        for ref_name, ref_file in ref_files.items():
            ref_path = REAL_DATA_DIR / ref_file
            if ref_path.exists():
                ref_img = load_tif_rgb(ref_path)
                extra_refs[ref_name] = cv2.resize(ref_img, (MODEL_INPUT_SIZE, MODEL_INPUT_SIZE),
                                                  interpolation=cv2.INTER_AREA)

        # Display inputs resized to 512 for consistent layout
        flash_disp = cv2.resize(flash_full, (MODEL_INPUT_SIZE, MODEL_INPUT_SIZE),
                                interpolation=cv2.INTER_AREA)
        noflash_disp = cv2.resize(noflash_full, (MODEL_INPUT_SIZE, MODEL_INPUT_SIZE),
                                  interpolation=cv2.INTER_AREA)

        save_comparison(
            out_dir / f"real_{scene_name}.png",
            f"Real: {scene_name} ({flash_full.shape[1]}x{flash_full.shape[0]} -> {MODEL_INPUT_SIZE})",
            flash_disp,
            noflash_disp,
            output_np,
            gates,
